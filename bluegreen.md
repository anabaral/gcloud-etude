...
- 변경은 Argocd 에 의해 인지되며 (자동 혹은 반자동)  서비스가 새 배포를 가리키게 되며 이 배포가 이제 블루(blue)배포본이 됨
- 변경이력은 git 저장소에서 관리됨
...
마이그레이션 때와 마찬가지로 프라이빗 저장소에 이미지를 부어 놓음. 공교롭게도 모든 이미지 버전이 갱신된 상태.
```
# 최신 helm 버전 확인 : 9.5.3
...
위에서 마이너 업그레이드라 할지라도 플러그인 업그레이드 같이 공유 볼륨이 변경되면서 기존 시스템 서비스에 중단이 가능할 때는 같은 저장소를 쓰면 
블루그린 배포의 의미가 없음. 그럴 경우엔 
- 새 저장소를 만들고 
- 이 저장소에 붙인 새 wordpress 를 설치하고
- 플러그인도 똑같이 설치하고
- 저장소 내용을 선별(!) 복제
해서 업그레이드 외의 구성을 최대한 같게 하는 절차를 밟아야 함.

DB 변경도 있으면 위의 절차에 새 DB 인스턴스 추가 및 여러 작업이 더해질 텐데 이 정도면 범위를 한참 넘으므로 넘어가자..
